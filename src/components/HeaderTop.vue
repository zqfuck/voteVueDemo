<template>
  <div>
    ...
    <div class="countBox">
      <div class="countKind">
        <p>参赛数</p>
        <p class="colorRed totaljoin">{{option_num}}</p>
        <span></span>
      </div>
      <div class="countKind">
        <p>投票数</p>
        <p class="colorRed totalVote">{{total_votes}}</p>
        <span></span>
      </div>
      <div class="countKind">
        <p>访问量</p>
        <p class="colorRed totalSee">{{click_num}}</p>
      </div>
    </div>
    <div class="timeEnd">
    ...
    detail () {
      ...
      api.getDetail(data).then(res => {
        if (res.state.rc >= 0) {
          ...
          that.click_num = res.data.click_num
          that.total_votes = res.data.total_votes
